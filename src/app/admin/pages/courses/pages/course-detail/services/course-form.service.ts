...
export class CourseFormService {
  ...
  async saveCourse(
    ...
    try {
      ...
      if (status === 'publish') {
        const exists = await this.db.docExists(`draft-courses/${formCourse.id}`);
      ...
      const normCourse = () => {
        return assignWith({}, formCourse, 
          (_, value) => typeof value == 'undefined' ? deleteField() : value);
      }
      ...
      if (!exists) await this.db.upsert<Course>(`courses/${formCourse.id}`, normCourse());
      else await this.db.upsert<Course>(`courses/${formCourse.id}`, { hide: formCourse.hide });
  
      if (originalCourse && formCourse.id !== originalCourse.id) {
        console.warn(`DELETING COURSE... -> Substituting '${originalCourse.id}' with '${formCourse.id}'`);
